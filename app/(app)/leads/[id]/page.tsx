    ...
    <div className="p-12 max-w-5xl">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <span
            className={`px-3 py-1.5 rounded-full text-sm font-semibold ${colorEstadoActual.bg} ${colorEstadoActual.text} flex-shrink-0`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="bg-white rounded-2xl border border-stone-200 p-6">
            ...
            <div className="space-y-3">
              <DatoFila icono={<Phone size={16} />} label="Teléfono">
                ...
                >
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ml-3 inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded border border-emerald-200 text-emerald-700 hover:bg-emerald-50"
                >
                  WhatsApp
                </a>
              </DatoFila>
              ...
              {lead.wp_nombre ? (
                <DatoFila icono={<Users size={16} />} label="Wedding Planner">
                  ...
                  {lead.wp_verificado === false ? (
                    <span className="ml-2 text-[10px] uppercase font-semibold px-1.5 py-0.5 bg-amber-100 text-amber-800 rounded">
                      sin verificar
                    </span>
                  ) : null}
                </DatoFila>
            ...
            {lead.mensaje_inicial ? (
              <div className="mt-5 pt-5 border-t border-stone-100">
                <div className="text-xs uppercase tracking-wider text-stone-500 mb-2 font-semibold">
                  Mensaje inicial
                </div>
          ...
          <section className="bg-white rounded-2xl border border-stone-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <DatoCompacto
                ...
                label="Tipo"
                valor={lead.tipo_evento ? TIPO_EVENTO_LABELS[lead.tipo_evento] : '—'}
              />
              <DatoCompacto
                icono={<Users size={16} />}
                label="Pax"
                valor={lead.pax ? `${lead.pax} personas` : '—'}
              />
              <DatoCompacto
                icono={<Calendar size={16} />}
                label="Fecha"
                valor={formatearFechaEvento(lead.fecha_evento)}
              />
              <DatoCompacto
          ...
          <section className="bg-white rounded-2xl border border-stone-200 p-6">
            ...
            {notas.length === 0 ? (
              <div className="text-sm text-stone-400 text-center py-6 bg-stone-50 rounded-lg">
                Sin notas todavía
              </div>
            ) : (
              <div className="space-y-3">
            ...
            {/* Placeholder para agregar nota (lo haremos en paso 3.5) */}
            <div className="mt-4 pt-4 border-t border-stone-100">
              <button
                disabled
                className="w-full bg-stone-100 text-stone-400 py-2 rounded-lg text-sm cursor-not-allowed"
        ...
        {/* Columna derecha: Acciones y datos secundarios */}
        <div className="space-y-6">
          {/* Ejecutivo asignado */}
          <section className="bg-white rounded-2xl border border-stone-200 p-6">
            <h2 className="text-xs uppercase tracking-wider text-stone-500 mb-3 font-semibold">
              Ejecutivo asignado
            </h2>
            {lead.ejecutivo_nombre ? (
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ backgroundColor: lead.ejecutivo_color ?? '#78716c' }}
                >
          ...
          {/* Cambiar estatus (placeholder, lo haremos en paso 3.3) */}
          <section className="bg-white rounded-2xl border border-stone-200 p-6">
            <h2 className="text-xs uppercase tracking-wider text-stone-500 mb-3 font-semibold">
              Cambiar estatus
            </h2>
          ...
          {puedeGenerarCotizacion ? (
            <section className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
              <h2 className="text-xs uppercase tracking-wider text-amber-800 mb-2 font-semibold">
                Acción rápida
              </h2>
              <p className="text-sm text-stone-700 mb-3">
                Genera una cotización con los datos de este lead.
              </p>
              <button
                disabled
                className="w-full bg-stone-200 text-stone-400 py-2.5 rounded-lg font-medium cursor-not-allowed"
                title="Disponible en la Fase 8"
              >
                + Generar cotización
              </button>
          ...
          {/* Si está perdido, mostrar la razón */}
          {lead.estado === 'PERDIDO' && lead.razon_perdida ? (
            <section className="bg-rose-50 border border-rose-200 rounded-2xl p-6">
              <h2 className="text-xs uppercase tracking-wider text-rose-800 mb-2 font-semibold">
                Razón de pérdida
              </h2>
              <div className="text-sm text-rose-900 font-medium">
                {RAZON_PERDIDA_LABELS[lead.razon_perdida]}
              </div>
              {lead.razon_perdida_detalle ? (
                <div className="text-xs text-rose-700 mt-2 italic">
                  &ldquo;{lead.razon_perdida_detalle}&rdquo;
                </div>
              ) : null}
            </section>
          ) : null}
